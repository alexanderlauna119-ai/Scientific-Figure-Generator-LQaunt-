...
class InteractivePlane(tk.Toplevel): 
    ...
    def __init__(self, master=None, title="Geometry Canvas"): 
        ...
        self._build_menu() 
        # Optional: keyboard shortcut to open Data Grid quickly 
        try: 
            self.bind_all("<Control-d>", lambda e: self._open_data_grid()) 
        except Exception: 
            pass 
    # ------------------------------------------------------------ 
    # SCROLL UTILS 
    ...
    def _build_menu(self): 
        ...
        # >>> NEW: DATA MENU <<< 
        md = tk.Menu(menu, tearoff=0) 
        menu.add_cascade(label="Data", menu=md) 
        md.add_command(label="Open Data Grid…", command=self._open_data_grid) 
    # ------------------------------------------------------------ 
    # >>> NEW: Excel-style Data Grid 
    # ------------------------------------------------------------ 
    def _open_data_grid(self): 
        """Open a simple Excel-style grid to type/paste X,Y pairs and push them as points.""" 
        top = tk.Toplevel(self) 
        top.title("Data Grid — X,Y to Points") 
        top.transient(self) 
        top.grab_set() 
        # ----- Table (Treeview) ----- 
        ...
        tv.heading("x", text="X") 
        tv.heading("y", text="Y") 
        tv.column("x", width=120, anchor="center") 
        tv.column("y", width=120, anchor="center") 
        yscroll = ttk.Scrollbar(table_frame, orient="vertical", command=tv.yview) 
        ...
        edit_entry = ttk.Entry(tv, textvariable=edit_var) 
        ...
        def _start_edit(event): 
            ...
            col_id = tv.identify_column(event.x)  # e.g. '#1' 
            ...
            edit_var.set(vals[col_index] if col_index < len(vals) else "") 
            edit_entry.place(x=x, y=y, width=w, height=h) 
            edit_entry.focus_set() 
            def _commit_edit(event=None): 
                vals[col_index] = edit_var.get() 
                tv.item(row_id, values=vals) 
                edit_entry.place_forget() 
            def _cancel_edit(event=None): 
                edit_entry.place_forget() 
            edit_entry.bind("<Return>", _commit_edit) 
            edit_entry.bind("<Escape>", _cancel_edit) 
            edit_entry.bind("<FocusOut>", _commit_edit) 
        tv.bind("<Double-1>", _start_edit) 
        # ----- Buttons ----- 
        btns = ttk.Frame(top) 
        ...
        def clear_table(): 
            for iid in tv.get_children(): 
                tv.delete(iid) 
        def paste_clipboard(): 
            """Paste text as rows. Accepts comma/tab/space-separated; multi-line blocks.""" 
            try: 
            ...
            for ln in lines:
                ...
                if parts is None:
                    parts = ln.split() 
                if len(parts) >= 2:
                    tv.insert("", "end", values=(parts[0], parts[1])) 
        def run_points(): 
            """Read X,Y from the table and add points to canvas. Skips non-numeric rows.""" 
            edit_entry.place_forget()  # commit any active edit 
            def to_float(s): 
                try: 
                    return float(s) 
                except Exception:
            ...
            for iid in tv.get_children(): 
                ...
                added += 1 
            self.canvas.draw_idle() 
            self.status.set(f"Added {added} point(s) from Data Grid.") 
        ttk.Button(btns, text="Add Row", command=add_row).pack(side="left") 
        ttk.Button(btns, text="Delete Row", command=delete_row).pack(side="left", padx=(6, 0)) 
        ttk.Button(btns, text="Paste", command=paste_clipboard).pack(side="left", padx=(6, 0)) 
        ttk.Button(btns, text="Clear Table", command=clear_table).pack(side="left", padx=(6, 0)) 
        ttk.Button(btns, text="Run", command=run_points).pack(side="right") 
        # Optional: allow Ctrl+V paste on the table 
        tv.bind("<Control-v>", lambda e: (paste_clipboard(), "break")) 
        tv.bind("<Command-v>", lambda e: (paste_clipboard(), "break"))  # macOS 
    # ------------------------------------------------------------ 
    ...
    def _dialog_add_point(self): 
        ...
        win.grab_set(); win.wait_window(win) 
        if not out["ok"]:
            return 
        try: 
    ...
    def _update_registry_from_circle(self, c): 
        for oid, data in self.registry.items(): 
            if data["ref"] is c and data["type"] == "circle": 
                x, y = c.center; data["props"]["x"], data["props"]["y"] = x, y 
    ...
    def _run_single_model_token(self, tok: str): 
        ...
        self.status.set("Unknown model: '%s'. Supported: linear, polyN, exponential, logarithmic, power, ridge, lasso, elasticnet, svr, tree, forest, knn" % tok) 
    # ============================================================ 
    ...
    def _expand_function_keyword(self, s: str) -> str: 
        ...
        See original commentary for supported shorthands. 
        """ 
        import re 
        t = s.strip().lower() 
        if t.startswith("y="):
    ...
    def _evaluate_function_expression(self, expr: str, x: np.ndarray, use_degrees: bool = False): 
        ...
        if s.lower().startswith("y="):
            s = s[2:].strip() 
        # Insert implicit multiplication before other rewrites 
